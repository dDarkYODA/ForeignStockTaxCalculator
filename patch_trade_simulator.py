...
def main():
    ...
    old_currency_logic = """        # Calculate cost in INR for the matched shares
        cost_fx_rate = fx_service.get_tt_buy_rate(lot.currency if hasattr(lot, 'currency') else 'USD', lot.date)
        lot_cost_inr = shares_matched * lot.price * cost_fx_rate"""

    new_currency_logic = """        # Calculate cost in INR for the matched shares prorated from lot total cost_inr
